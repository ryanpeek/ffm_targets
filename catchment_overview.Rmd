...
nhdplus <- subset_nhdplus(comids = as.integer(final_comids),
                          output_file = glue(here("data_output/lsh_nhdplus_raw_vaa.gpkg")),
...
# load data
db <- here("data_output/lsh_nhdplus_raw_vaa.gpkg")
...
rm(coms_missing, db, final_comids, db, loi_comids, comid_trim, flownet_sort,
...
#mapview(catch_trim, alpha=0.4, zcol="area_weight") + 
#  mapview(flowlines_net, color="cyan4")
...
catch_final <- left_join(catch_trim, st_drop_geometry(catchment_area) %>%
                          select(ID, totda, nhdptotda),
                        by=c("comid"="ID"))

# now add back to lsh_flownet
flownet_final <- flowlines_net %>%
  select(-c( totdasqkm)) %>%
  left_join(., st_drop_geometry(catch_final),
            by=c("ID"="comid")) %>%
...
write_rds(comlist, file="data_output/cleaned_full_lshasta_comid_accum_list.rds")
write_rds(flowlines_net, file = "data_output/cleaned_full_lshasta_nhd_flownet.rds")
write_rds(catch_trim, file="data_output/cleaned_full_lshasta_nhd_catchments.rds")
```
